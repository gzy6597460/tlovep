...
class Wechat
{
	...
	const MSGTYPE_LOCATION 			= 'location';			// 位置消息
	const MSGTYPE_LINK 				= 'link';				// 连接消息
	const MSGTYPE_EVENT 			= 'event';				// 事件消息
	const MSGTYPE_MUSIC 			= 'music';				// 音乐消息
	const MSGTYPE_NEWS 				= 'news';				// 图文消息
	const MSGTYPE_VOICE 			= 'voice';				// 音频消息
	const MSGTYPE_VIDEO 			= 'video';				// 视频消息
	const MSGTYPE_SHORTVIDEO 		= 'shortvideo';			// 短视频
	// 事件
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * }
     */
    public function getCardIdList($offset=0,$count=50,$status_list=[]) {
        if ($count>50)
            $count = 50;
        $data = array(
            'offset' => $offset,
    ...
    /**
     * 会员卡交易
     * 会员卡交易后每次积星及余额变更需通过接口通知微信，便于后续消息通知及其他扩展功能。
